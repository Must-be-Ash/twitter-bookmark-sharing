...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    for (const user of users) {
      ...
      const twitterClient = new TwitterApi(user.accessToken || process.env.TWITTER_BEARER_TOKEN!);
      ...
      try {
        ...
        const bookmarks: BookmarksResponse = {
          ...
          meta: { result_count: bookmarksResponse.data.meta?.result_count || 0 }
        };
        ...
        if (bookmarks.tweets.length > 0) {
          bookmarkListItems = bookmarks.tweets.map((tweet: TweetV2) => {
            const author = bookmarks.users.find((u: UserV2) => u.id === tweet.author_id);
            return `
              <li>
        ...
        console.error(`Error processing bookmarks for user ${user.username}:`, error);
